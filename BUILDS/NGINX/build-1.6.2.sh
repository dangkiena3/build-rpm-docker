...
./configure  --prefix=/usr/local/nginx --sbin-path=/usr/local/nginx/sbin/nginx --conf-path=/usr/local/nginx/conf/nginx.conf --http-log-path=/var/log/nginx/access.log --error-log-path=/var/log/nginx/error.log --without-mail_pop3_module --without-mail_imap_module --without-mail_smtp_module --without-http_split_clients_module --without-http_uwsgi_module --without-http_scgi_module --with-http_ssl_module --with-http_realip_module --with-http_gzip_static_module --with-http_secure_link_module --with-http_dav_module --with-http_stub_status_module --with-http_geoip_module --add-module=../ngx_http_log_request_speed --with-debug --with-file-aio --with-http_mp4_module --with-http_perl_module --with-ld-opt="-Wl,-E" --add-module=../echo-nginx-module/ --add-module=../ngx_http_redis
...
fpm -s dir -t rpm -n nginx -v ${VERSION} -p nginx-VERSION.rpm --url "https://appota.com" --description "Nginx ${VERSION}" --vendor "Kiennd" -d pcre -d pcre-devel  -d zip -d unzip -d gcc -d gcc-c++ -d openssl -d openssl-devel -d zlib -d gd -d freetype -d freetype-devel -d autoconf -d openldap -d openldap-devel -d bzip2-devel -d libpng -d libpng-devel -d GeoIP -d GeoIP-devel usr etc var 
